function copyJobFilesIfFinished(jobStateTimer, ~, scheduler, job)
%COPYJOBFILESIFFINISHED Copy the job files of a finished job to the
% client machine.

% Copyright 2010 MathWorks, Inc.

disp('copyJobFilesIfFinished')


% Make sure that the job is valid
if isempty(job) % ?? not sure|| isempty(strfind(class(job), 'distcomp'))
...
localDataLocation = scheduler.JobStorageLocation; %scheduler.DataLocation;
jobLocation = ['Job' num2str(job.ID)];%job.Name; %job.pGetEntityLocation;
...
if strcmp(state, 'finished')
    iStopAndDeleteTimer(jobStateTimer);
    slurmGetJobState(scheduler, job, 'finished'); % delete job from jobsToMonitorNames
    try
