classdef horizontalMarker < handle
    %HORIZONTALMARKER creates a horizontal marker in an axes. Additional a
    %slider is generated
    %   OBJ = HORIZONTALMARKER(AXES, 'parameter', value, ...) creates a
    %   horizontal marker in the AXES object
    %   Possible parameters:
    ...
    methods
        function obj = horizontalMarker(varargin)
            if (nargin)
    ...
    methods
        function set.min(obj, min)
            assert(isnumeric(min) && isfinite(min), ...
                'Gui:horizontalMarker:minNonNumericOrInfinite', ...
        ...
        function set.value(obj, value)
            assert(isnumeric(value) && isfinite(value), ...
                'Gui:horizontalMarker:valueNonNumericOrInfinite', ...
                'value must be numeric an finite');
            assert(value >= obj.min, ...
                'Gui:horizontalMarker:valueSmallerThanMin', ...
                'value must be bigger than min');
            assert(value <= obj.max, ...
                'Gui:horizontalMarker:valueBiggerThanMax', ...
        ...
        function set.max(obj, max)
            assert(isnumeric(max) && isfinite(max), ...
                'Gui:horizontalMarker:maxNonNumericOrInfinite', ...
                'maximal value must be numeric and finite');
            if (max ~= obj.max)
                obj.max = max;
