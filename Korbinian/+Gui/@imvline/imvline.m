classdef imvline < handle
    %IMVLINE Create draggable, vertical line
    ...
    methods
        function obj = imvline(hparent, position, varargin)
            if (nargin < 1)
                hparent = gca;
            end
            if (nargin < 2)
        ...
        function set.position(obj, x)
            assert(all(size(x) == 1) && isnumeric(x), ...
                'images:imvline:invalidPositionSizeOrClass', ...
